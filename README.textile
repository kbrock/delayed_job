...
NOTE: DIFFERENCES FROM standard delayed_job

* priority is inverse. Lower is better. (thank to http://www.elevatedrails.com/articles/2009/11/04/speeding-up-delayed-job/ )
* completed and failed tasks are stored in the databse (per http://github.com/obvio171/delayed_job )
* summary records are stored for reporting purposes (so we don't have to keep around failed/completed jobs)
...
To use the rake task from a vendored gem, you'll need to add the following to your Rakefile:

<pre>
require(File.join(RAILS_ROOT, 'vendor', 'gems', 'collectiveidea-delayed_job-1.8.2', 'lib', 'delayed_job'))
</pre>
...
This is not limited to your own methods. You can also call ActiveMailer to deliver email in the background:
